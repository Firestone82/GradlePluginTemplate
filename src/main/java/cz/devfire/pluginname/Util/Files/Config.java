...
public final class Config extends YamlConfiguration {
    ...
    private String[] ignoredSections = null;
    private File file = null;
    private Plugin plugin = null;


    public static Config loadConfig(Plugin plugin, String pluginPath, String serverPath, String... ignoredSections) {
        return loadConfig(plugin,pluginPath,serverPath,false,ignoredSections);
    }

    public static Config loadConfig(Plugin plugin, String pluginPath, String serverPath, boolean silent, String... ignoredSections) {
        ...
        Config cfg = Config.loadConfiguration(f);
        ...
        cfg.plugin = plugin;
        cfg.silent = silent;
        cfg.syncWithConfig(f,plugin.getResource(pluginPath),ignoredSections);

        if (!silent) {
            Bukkit.getConsoleSender().sendMessage("§a - Loading " + cfg.getName() + "... §eSuccessful!");
        }
    ...
    public void syncWithConfig(File file, InputStream resource, String... ignoredSections){
        ...
        Config cfg = loadConfiguration(resource,file.getName());
     ...
     * @return A new instance of CommentedConfiguration contains all the data (keys, values and comments).
     */
    public static Config loadConfiguration(InputStream inputStream, String name) {
        return loadConfiguration(new InputStreamReader(inputStream, StandardCharsets.UTF_8), name);
    }
    ...
    public static Config loadConfiguration(Reader reader, String name) {
        ...
        try(BufferedReader bufferedReader = reader instanceof BufferedReader ? (BufferedReader)reader : new BufferedReader(reader)){
            ...
            config.loadFromString(contents.toString(),name);
        } catch (IOException | InvalidConfigurationException ex) {
            Bukkit.getConsoleSender().sendMessage("§c - Loading " + name + "... §cFailed!");
            config.flagAsFailed();
            ex.printStackTrace();
        }
